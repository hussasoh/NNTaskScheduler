...
def generate_schedule(tasks, daily_availability):
    ...
    # Sort tasks by priority (higher priority first) and deadline (earlier first)
...
def add_task():
    try:
        ...
        priority_text = priority_combobox.get()  # Fetch priority from combobox
        priority = PRIORITY_MAP[priority_text]  # Map priority text to integer value
...
def renumber_tasks():
    ...
    for i, task in enumerate(tasks):
        task_id, deadline, priority, time_needed, predicted_time_slot, description = task
        new_task_id = i + 1
        tasks[i] = (new_task_id, deadline, priority, time_needed, predicted_time_slot, description)
        tasks_listbox.insert(tk.END, f"Task {new_task_id}: {description} | Time Needed: {format_time_slot(time_needed)} | Predicted Time Slot: {format_time_slot(predicted_time_slot)}")
