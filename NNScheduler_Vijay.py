...
def generate_realistic_data(num_samples=1000):
    ...
    availability = np.random.randint(1, 10, num_samples)  # availability slots in hours per day
    ...
    data['time_slots'] = (data['deadlines'] / data['availability']) * data['priorities']
...
def prepare_data(data):
    features = data[['tasks', 'deadlines', 'priorities', 'availability']]
...
# Scheduling function
def generate_schedule(tasks, availability_per_day):
    total_time_available = availability_per_day * 60  # Convert hours to minutes
    schedule = []
    current_time = 0

    for task in tasks:
        task_id, deadline, priority, availability, duration_hours = task
        duration_minutes = int(duration_hours * 60)
        if current_time + duration_minutes <= total_time_available:
            start_time = current_time
            end_time = current_time + duration_minutes
            schedule.append((task_id, start_time, end_time))
...
def add_task():
    try:
        task = int(task_entry.get())
        deadline = int(deadline_entry.get())
        priority = int(priority_combobox.get())
        availability = int(availability_entry.get())

        if any([task <= 0, deadline <= 0, priority <= 0, availability <= 0]):
            raise ValueError("Inputs must be positive integers.")

        new_task = np.array([[task, deadline, priority, availability]])
        predicted_time_slot = predict_task_time_slot(new_task)[0][0]
        tasks.append((task, deadline, priority, availability, predicted_time_slot))
        tasks_listbox.insert(tk.END, f"Task {task}: {format_time_slot(predicted_time_slot)}")
...
def generate_daily_schedule():
    try:
        ...
        for task_id, start_time, end_time in schedule:
            ...
            schedule_text += (f"Task {task_id}: {start_hour:02}:{start_minute:02} - {end_hour:02}:{end_minute:02}\n")

        messagebox.showinfo("Daily Schedule", schedule_text)
    except ValueError as ve:
        messagebox.showerror("Input Error", f"Invalid input: {ve}")
    except Exception as e:
        messagebox.showerror("Error", f"An unexpected error occurred: {e}")
...
tk.Label(root, text="Task ID").grid(row=0, column=0, padx=10, pady=5)
tk.Label(root, text="Deadline (days)").grid(row=1, column=0, padx=10, pady=5)
tk.Label(root, text="Priority").grid(row=2, column=0, padx=10, pady=5)
tk.Label(root, text="Availability (hours)").grid(row=3, column=0, padx=10, pady=5)
tk.Label(root, text="Daily Availability (hours)").grid(row=4, column=0, padx=10, pady=5)

task_entry = tk.Entry(root)
deadline_entry = tk.Entry(root)
priority_combobox = ttk.Combobox(root, values=[1, 2, 3, 4])
availability_entry = tk.Entry(root)
daily_availability_entry = tk.Entry(root)
...
daily_availability_entry.grid(row=4, column=1, padx=10, pady=5)

tasks_listbox = tk.Listbox(root)
tasks_listbox.grid(row=0, column=2, rowspan=5, padx=10, pady=5)

tk.Button(root, text='Add Task', command=add_task).grid(row=5, column=0, sticky=tk.W, pady=10, padx=10)
tk.Button(root, text='Generate Schedule', command=generate_daily_schedule).grid(row=5, column=1, sticky=tk.E, pady=10,
                                                                                padx=10)
tk.Button(root, text='Quit', command=root.quit).grid(row=5, column=2, sticky=tk.E, pady=10, padx=10)
